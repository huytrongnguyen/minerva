...
from shared import string_utils, file_utils

from ..python.adapter import execute_query
...
def save_data(data: DataFrame, model: ModelSettings, vars: dict[str, Any], job_settings: JobSettings):
  if model.type == 'jdbc':
    save_data_with_jdbc(data, model, vars, job_settings)
  else:
    path = string_utils.parse(model.location, vars)
...
def save_data_with_jdbc(data: DataFrame, model: ModelSettings, vars: dict[str, Any], job_settings: JobSettings):
  path = string_utils.parse(model.location, vars)
  cred = file_utils.load_json(path)
  options = {
    'driver': cred['driver'],
    'url': cred['url'],
    'dbtable': model.name,

...
def transform_dataset(source_data: DataFrame, target_model: ModelSettings, vars: dict[str, Any]) -> DataFrame:
  ...
  if target_model.query is not None and len(target_model.query) > 0:
    target_data = reduce(lambda data, func: invoke_query_function(data, func), target_model.query, target_data)

  if target_model.aggregation:
    target_data = aggregate(target_data, AggregationSettings(**target_model.aggregation))
...
def aggregate(data: DataFrame, aggregation: AggregationSettings) -> DataFrame:
  if aggregation.type == 'group_by': return group_by(data, aggregation)
  elif aggregation.type == 'partition_by': return partition_by(data, aggregation)
  else:
    print(f'"{aggregation.type}" is not defined.')
    return data

def group_by(data: DataFrame, aggregation: AggregationSettings) -> DataFrame:
  agg_columns: list[Column] = [invoke_group_by_function(ColumnSettings(**measure)) for measure in aggregation.measures]
  return data.groupby(aggregation.dimensions).agg(*agg_columns)

def partition_by(data: DataFrame, aggregation: AggregationSettings) -> DataFrame:
  partition = Window.partitionBy(*aggregation.dimensions)
  order_columns = [invoke_order_by_column(func) for func in aggregation.order_by]
